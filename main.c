  ...
  while (!quit) {
	...
	if(event.key.keysym.scancode == SDL_SCANCODE_F5 && !selectedTextInput) {
	  ...
	  case(SDL_SCANCODE_UP): {
		...
		if (data->tile->customWalls[data->side].buf[1] + manipulationStep <= bBlockH) {
		  data->tile->customWalls[data->side].buf[1] += manipulationStep;
		  data->tile->customWalls[data->side].buf[6] += manipulationStep;
		  data->tile->customWalls[data->side].buf[16] += manipulationStep;

		  float uvStep = (manipulationStep * 10) / 2;
		  
		  data->tile->customWalls[data->side].buf[4] += uvStep;
		  data->tile->customWalls[data->side].buf[9] += uvStep;
		  data->tile->customWalls[data->side].buf[19] += uvStep;

		  vec2i oppositeTile = { 0 };
		  int oppositeSide = -1;

		  if (oppositeTileTo((vec2i) { data->grid.x, data->grid.z }, data->side, &oppositeTile, &oppositeSide)) {
		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[1] += manipulationStep;
		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[6] += manipulationStep;
		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[16] += manipulationStep;

		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[4] += uvStep;
		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[9] += uvStep;
		    grid[data->grid.y][oppositeTile.z][oppositeTile.x].customWalls[oppositeSide].buf[19] += uvStep;
		  } 
		}
	      }else{
	  ...
	  case(SDL_SCANCODE_DOWN): {
		...
		if (currentKeyStates[SDL_SCANCODE_LCTRL] && block->vertexes[11] - manipulationStep >= 0) {
	      ...
	      if(currentKeyStates[SDL_SCANCODE_LCTRL]){
		if (data->tile->customWalls[data->side].buf[1] - manipulationStep >= 0.01f) {
		  data->tile->customWalls[data->side].buf[1] -= manipulationStep;
		  data->tile->customWalls[data->side].buf[6] -= manipulationStep;
		  data->tile->customWalls[data->side].buf[16] -= manipulationStep;
	  ...
	  case(SDL_SCANCODE_H): {
	    ...
	    if(mouse.selectedType == mouseWallT && currentKeyStates[SDL_SCANCODE_LCTRL]){
	      ...
	      Side selSide = data->side;
	      
	      vec2i oppositeTile;
	      Side oppositeSide;
	  
	      if (oppositeTileTo((vec2i) { data->grid.x, data->grid.z }, data->side, & oppositeTile, & oppositeSide)) {
		oppositeGrid = &grid[data->grid.y][oppositeTile.z][oppositeTile.x];

		if (!block) {
		  selSide = oppositeSide;
		...
		if (block && block->type == roofBlockT) {
		  ...
		  Side curSide = mappedSideToAngleAgainstClock[selSide][block->rotateAngle/90];
		  ...
		  if (!data->tile->customWalls[data->side].alligned && !data->tile->customWalls[data->side].alligned) {
			...
			data->tile->customWalls[data->side].alligned = true;
		      ...
		      if(newH > 0.0f || backV > 0.0f){
			if (oppositeGrid) {
			  oppositeGrid->customWalls[oppositeSide].buf[1] = newH; //newH;
			  oppositeGrid->customWalls[oppositeSide].buf[4] = newV;//newV;

			  oppositeGrid->customWalls[oppositeSide].buf[6] = backH;
			  oppositeGrid->customWalls[oppositeSide].buf[9] = backV;

			  oppositeGrid->customWalls[oppositeSide].buf[16] = backH;
			  oppositeGrid->customWalls[oppositeSide].buf[19] = backV;
			...
			data->tile->customWalls[data->side].buf[6] = backH;
			data->tile->customWalls[data->side].buf[9] = backV;

			data->tile->customWalls[data->side].buf[16] = backH;
			data->tile->customWalls[data->side].buf[19] = backV;
	  ...
	  case(SDL_SCANCODE_DELETE): {
	    ...
	    if(mouse.brushType == mouseTextureBrushT && currentKeyStates[SDL_SCANCODE_LCTRL]){
	      if(mouse.selectedType == mouseWallT){
		WallMouseData* wallData = (WallMouseData*)mouse.selectedThing;

		wallData->tile->customWalls[wallData->side].txHidden = !wallData->tile->customWalls[wallData->side].txHidden;
	      }
		...
		if (mouse.focusedType == mouseModelT) {
		  ...
		  Plane* panel = (Plane*)mouse.focusedThing;
		  int index = 0;
	...
	for (int z = 0; z < gridZ; z++){
	  ...
	  GroundType type = valueIn(grid[curFloor][z][x].ground, 0);
    ...
    */
	...
	break;
	 ...
	 Plane* data = (Plane*)mouse.focusedThing;
