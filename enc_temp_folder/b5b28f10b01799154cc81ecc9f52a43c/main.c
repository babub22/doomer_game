...
int pointLightsSize;
const PointLight pointLightsDef = { .color = rgbToGl(253.0f, 244.0f, 220.0f), .constant = 1.0f, .linear = .09f, .quadratic = .032f };
...
int main(int argc, char* argv[]) {
  ...
  // lightning(main) shader
  modelLoc = glGetUniformLocation(shadersId[mainShader], "model");
  int projUni = glGetUniformLocation(shadersId[mainShader], "proj");
  int viewLoc = glGetUniformLocation(shadersId[mainShader], "view");
  /*
  int lightColor = glGetUniformLocation(shadersId[mainShader], "light.color");
  int lightPos = glGetUniformLocation(shadersId[mainShader], "light->pos");
  int lightDir = glGetUniformLocation(shadersId[mainShader], "light.dir");
  int lightRad = glGetUniformLocation(shadersId[mainShader], "light.rad");
  int lightCutOff = glGetUniformLocation(shadersId[mainShader], "light.cutOff");
  
  int lightConstant = glGetUniformLocation(shadersId[mainShader], "light.constant");
  int lightLinear = glGetUniformLocation(shadersId[mainShader], "light.linear");
  int lightQuadratic = glGetUniformLocation(shadersId[mainShader], "light.quadratic");
*/

  // light source shader
  lightModelLoc = glGetUniformLocation(shadersId[lightSourceShader], "model");
  int lightProjUni = glGetUniformLocation(shadersId[lightSourceShader], "proj");
  int lightViewLoc = glGetUniformLocation(shadersId[lightSourceShader], "view");

  // hud shader
  int orthoLoc = glGetUniformLocation(shadersId[hudShader], "ortho");
  int hudColorLoc = glGetUniformLocation(shadersId[hudShader], "u_Color");
  
  int viewportLoc = glGetUniformLocation(shadersId[hudShader], "viewport");

  // border shader
    int borderColor = glGetUniformLocation(shadersId[borderShader], "borderColor");
    int borderModelLoc = glGetUniformLocation(shadersId[borderShader], "model");
    int borderProjUni = glGetUniformLocation(shadersId[borderShader], "proj");
    int borderViewLoc = glGetUniformLocation(shadersId[borderShader], "view");

    // screen shader
    int screenShaderDof = glGetUniformLocation(shadersId[screenShader], "dof");
        int screenShaderTime = glGetUniformLocation(shadersId[screenShader], "time");
  ...
  while (!quit) {
	...
	if(event.key.keysym.scancode == SDL_SCANCODE_F5 && !selectedTextInput) {
	  ...
	  case(SDL_SCANCODE_UP): {
	    if(mouse.focusedType == mousePointLightT){
	      const Uint8* currentKeyStates = SDL_GetKeyboardState(NULL);

	      PointLight* light = (PointLight*)mouse.focusedThing;
	  ...
	  case(SDL_SCANCODE_F): {
	    if(!mouse.focusedThing){
	      if(mouse.selectedType == mousePointLightT || mouse.selectedType == mousePlaneT || mouse.selectedType == mouseModelT){
...
//	    GL_DEPTH_BUFFER_BIT);
    ...
    // send proj and view mat to shader
    {
      ...
      glUniformMatrix4fv(borderProjUni, 1, GL_FALSE, proj.m);
      glUniformMatrix4fv(borderViewLoc, 1, GL_FALSE, view.m);
	...
	uniformInt(mainShader, "pointLightsSize", pointLightsSize);

	for(int i=0;i<pointLightsSize;i++){
	  //	  printf("%f %f %f,);
	  sprintf(buf, "pointLights[%i].pos", i);
	  uniformVec3(mainShader, buf, pointLights[i].pos); 

	  sprintf(buf, "pointLights[%i].color", i);
	  uniformVec3(mainShader, buf, pointLights[i].color);

	  sprintf(buf, "pointLights[%i].constant", i);
	...
	*/
	...
	glUniform3f(borderColor, redColor);
    ...
    glUniform1f(screenShaderDof, -(dofPercent - 1.0f));

    float seed = (float)(rand() % 1000 + 1) / 1000.0f;

    glUniform1f(screenShaderTime, seed); 
    ...
    */
	...
	break;
	 ...
	 break;
       }case(mousePointLightT):{
	  PointLight* light = (PointLight*)mouse.focusedThing;
	  sprintf(buf, "Focused point light: [%d]", light->id);
	...
	if(mouse.clickL){
	  ...
	  if(selectedIndex-1 == pointLightT){
	    pointLightsSize++;

	    if(!pointLights){
	      pointLights = malloc(sizeof(PointLight));
	    }else{
	      pointLights = realloc(pointLights, sizeof(PointLight) * pointLightsSize);
	    }
	    
	    memcpy(&pointLights[pointLightsSize-1], &pointLightsDef, sizeof(PointLight));
	    pointLights[pointLightsSize-1].pos = curCamera->pos;
	    pointLights[pointLightsSize-1].id = pointLightsSize-1;
	    
	    mouse.focusedType = mousePointLightT;
	    mouse.focusedThing = &pointLights[pointLightsSize-1];
	  }else if(selectedIndex-1 == dirLightT){
	    // ....
	  } 
...
void renderCube(vec3 pos){
  ...
  if(isIntersect){
      mouse.selectedType = mousePointLightT;
      mouse.selectedThing = NULL;
...
ModelInfo* loadOBJ(char* path, char* texturePath){
  ...
  for(int i=0;i<loadedModel->size * 8;i+=8){
    ...
    //    printf("%f %f %f \n", modelVerts[i + 5], modelVerts[i + 6], modelVerts[i + 7]);
