...
//	    /*
		...
		glBindVertexArray(d2GridTri.VAO);
		glBindBuffer(GL_ARRAY_BUFFER, d2GridTri.VBO);

		uniformMat4(lightSourceShader, "model", gridTriMat.m);
		uniformVec3(lightSourceShader, "color", (vec3) { redColor });
		    
		glUseProgram(shadersId[lightSourceShader]);
		glDrawArrays(GL_TRIANGLES, 0, d2GridTri.VBOsize);

		glBindVertexArray(d2GridPlayerPoint.VAO);
		glBindBuffer(GL_ARRAY_BUFFER, d2GridPlayerPoint.VBO);

		uniformMat4(lightSourceShader, "model", gridTriMat.m);
		uniformVec3(lightSourceShader, "color", (vec3) { cyan });
		    
		glUseProgram(shadersId[lightSourceShader]);
		glDrawArrays(GL_TRIANGLES, 0, d2GridPlayerPoint.VBOsize);
...
bool entityVsMeshes(vec3 entityPos, vec3* resPos){
    bool valid = false;
    uint32_t start = SDL_GetPerformanceCounter();
	...
	for(int i=0;i<tempAABBSize;++i){
		...
		for(int i3=0;i3<objectsInfo[infoId].meshes[i2].posBufSize/9; i3++) {
		    ...
		    // can be faster
		    float A = triArea2D(a,b,c);

		    float A1 = triArea2D(player2DPos,a,b);
		    float A2 = triArea2D(player2DPos,b,c);
		    float A3 = triArea2D(player2DPos,c,a);

		    if(fcmp(A,(A1+A2+A3))){
			vec3 a1 = {a.x, objectsInfo[infoId].meshes[i2].posBuf[index + 1], a.z};
			vec3 b1 = {b.x, objectsInfo[infoId].meshes[i2].posBuf[index + 4], b.z};
			vec3 c1 = {c.x, objectsInfo[infoId].meshes[i2].posBuf[index + 7], c.z};
				
			printf("\nIntersected with: v1(%f %f %f) v2(%f %f %f) v3(%f %f %f)\n",
			       argVec3(a1),argVec3(b1),argVec3(c1));
				
			printf("Player pos: v1(%f %f) \n", player2DPos.x, player2DPos.z);

			vec3 interPos = interpolate2dTo3d(a1,b1,c1,player2DPos);
...
//			tempModelsPos[tempModelsPosSize] = interPos;
//			tempModelsPosSize++;
...
//			break;
	...
	if(valid){
	    playerPoint = (vec4){argVec3(finalPos), 1.0f };
	    playerPoint = mulmatvec4(finalObjMat, playerPoint);

	    entityPos = (vec3){argVec3(playerPoint)};
	}
    }
    ...
    float delta = SDL_GetPerformanceCounter() - start;
    printf("entityVsMeshes: %f \n", delta/1000.0f);
